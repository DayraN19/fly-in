...
def find_short_path(start_hub, end_hub, all_hubs):
    ...
    while queue:
        ...
        connections = current_hub.connections if hasattr(current_hub,
                                                         'connections') else []
        for neighbor in connections:
            neighbor_name = neighbor.name if hasattr(neighbor,
                                                     'name') else neighbor
...
def get_dynamic_path(drone_current_hub, end_hub, all_hubs):
    ...
    while queue:
        ...
        for neighbor in connections:
            ...
            # Évite d'emprunter des hubs surchargés
            if neighbor_name != end_hub.name:
                current_drones = getattr(neighbor_hub,
                                         'current_drones_count', 0)
                max_drones = getattr(neighbor_hub, 'max_drones', 1)
                if current_drones >= max_drones:
                    move_cost += 10 * (current_drones - max_drones + 1)
